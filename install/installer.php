...
if(version_compare(PHP_VERSION, '5.3.0', '<')) {
	$errors[] = 'TestCMS requires PHP 5.3 or newer, your current environment is running PHP ' . PHP_VERSION;
}
...
if(empty($errors)) {
	...
	if($post['clean_urls']) {
		...
		if(file_exists('../.htaccess') === false) {
			...
			$warnings[] = 'It looks like you already have a htaccess file in place, to use clean URLs please copy and paste our sample htaccess.txt file, remember to update the RewriteBase option if you have installed TestCMS in a subfolder.';
		}
	}
}
